...
{
    public class ServiceTemplate : Template, ITemplate
    {
        ...
        {
            this.Output.File = System.IO.Path.GetFileNameWithoutExtension(this.Model.Parent.Name) + ".svc.cs";
            ...
            this.WriteLine("using System.ServiceModel.Web;");
            this.WriteLine("using System.ServiceModel.Activation;");
            ...
            foreach (var @namespace in this.Model.CodeElements.OfType<CodeNamespace>())
            {
                foreach (var entityClass in @namespace.Children.OfType<CodeClass2>())
                {
                    ...
                    {
                        this.WriteLine("[ServiceContract(Namespace = \"\")]");
                        this.WriteLine("[AspNetCompatibilityRequirements(RequirementsMode = AspNetCompatibilityRequirementsMode.Allowed)]");
                        this.WriteLine(string.Format("public class {0}Service", entityClass.Name));
                        using (ScopeController.Begin(this))
                        {
                            this.WriteLine("[OperationContract]");
                            this.WriteLine("[WebInvoke(Method = \"GET\",BodyStyle = WebMessageBodyStyle.Bare,ResponseFormat = WebMessageFormat.Json)]");
                            this.WriteLine("public string Hello()");
                            using (ScopeController.Begin(this))
                            {
                                this.WriteLine("return string.Format(\"Hello from {0}\", this);");
                            }
